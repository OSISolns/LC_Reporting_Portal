...
// ── Mini stat card ────────────────────────────────────────────────────────────
const HSFPStatCard = ({ title, value, icon, color, sub }) => (
...
// ── Recent pending row ────────────────────────────────────────────────────────
const AnalysisRow = ({ incident, navigate }) => {
...
const HSFPDashboard = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error('HSFP Dashboard load error:', e);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const analysisRate = total ? Math.round((approved.length / total) * 100) : 0;
  ...
  return (
    <div style={{ paddingBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem', position: 'relative' }}>
          <div>
            ...
            <h1 style={{ margin: 0, fontSize: '2.25rem', fontWeight: 800, color: '#fff', display: 'flex', alignItems: 'center', gap: '12px' }}>
              <ShieldCheck size={32} /> Safety Focal Portal
            </h1>
            <p style={{ margin: '10px 0 0', opacity: 0.8, fontSize: '1rem', fontWeight: 500 }}>
              Welcome, <strong>{user?.fullName}</strong> — Safety analytics and RCA workspace.
            </p>
          </div>
          <button onClick={load} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '0.75rem 1.25rem', backgroundColor: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '12px', color: '#fff', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', backdropFilter: 'blur(10px)' }}>
            <RefreshCw size={16} /> Sync Safety Records
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
        <HSFPStatCard title="Safety Incidents" value={total} icon={<AlertTriangle size={24} />} color="#b91c1c"
          sub={`${thisMonth.length} reported this month`} />
        <HSFPStatCard title="Awaiting Analysis" value={pending.length} icon={<Clock size={24} />} color="#92400e"
          sub={pending.length > 0 ? 'Urgent attention required' : 'Safety analysis up to date'} />
        <HSFPStatCard title="RCA Validated" value={approved.length} icon={<CheckCircle size={24} />} color="var(--success)"
          sub={`${analysisRate}% validation rate`} />
        <HSFPStatCard title="Safety Trend" value={thisMonth.length} icon={<Activity size={24} />} color="var(--primary)"
          sub="Monthly activity volume" />
      </div>
      ...
      <div style={{ marginBottom: '2rem', padding: '2rem', backgroundColor: '#ffffff', borderRadius: '20px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <span style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--primary-dark)' }}>Root Cause Analysis Completion</span>
          <span style={{ fontWeight: 800, fontSize: '1.25rem', color: analysisRate >= 80 ? 'var(--success)' : analysisRate >= 50 ? '#ca8a04' : '#b91c1c' }}>{analysisRate}%</span>
        </div>
        <div style={{ height: '14px', backgroundColor: '#f1f5f9', borderRadius: '99px', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${analysisRate}%`, background: 'linear-gradient(90deg, #16a34a 0%, #22c55e 100%)', borderRadius: '99px', transition: 'width 1.2s cubic-bezier(0.4, 0, 0.2, 1)' }} />
        </div>
        <div style={{ display: 'flex', gap: '2rem', marginTop: '14px' }}>
          {[{ label: 'Validated (RCA Complete)', n: approved.length, color: 'var(--success)' }, { label: 'Pending Safety Review', n: pending.length, color: '#f59e0b' }].map(k => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '2rem', marginBottom: '2rem' }}>
        <div style={{ backgroundColor: '#ffffff', borderRadius: '20px', border: '1px solid #e2e8f0', padding: '2rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
          <h3 style={{ margin: '0 0 1.5rem', fontSize: '1.1rem', fontWeight: 800, color: 'var(--primary-dark)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Zap size={20} style={{ color: '#f59e0b' }} /> Safety Classifications
          </h3>
        ...
        <div style={{ backgroundColor: '#ffffff', borderRadius: '20px', border: '1px solid #e2e8f0', padding: '2rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
          <h3 style={{ margin: '0 0 1.5rem', fontSize: '1.1rem', fontWeight: 800, color: 'var(--primary-dark)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Layers size={20} style={{ color: 'var(--primary)' }} /> Risk Distribution by Dept.
          </h3>
      ...
      <div style={{ backgroundColor: '#ffffff', borderRadius: '20px', border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '2rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
        <div style={{ padding: '1.5rem 2rem', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#f8fafc' }}>
          <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, color: 'var(--primary-dark)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <HeartPulse size={20} style={{ color: '#b91c1c' }} />
            Safety Workspace: Pending RCA
            {pending.length > 0 && (
              <span style={{ padding: '4px 12px', borderRadius: '99px', backgroundColor: '#fee2e2', color: '#b91c1c', fontSize: '0.75rem', fontWeight: 800 }}>{pending.length} ACTIONS</span>
            )}
          </h3>
          <button onClick={() => navigate('/incidents')}
            style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem', color: 'var(--primary)', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer' }}>
            Inspect Incident Registry <ExternalLink size={16} />
          </button>
        </div>
        <div>
          {pendingQueue.length === 0 ? (
            <div style={{ padding: '4rem 2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
              ...
              <p style={{ fontWeight: 800, fontSize: '1.2rem', color: 'var(--primary-dark)' }}>No Safety Backlog</p>
              <p style={{ marginTop: '4px' }}>All reported clinical incidents have completed safety analysis.</p>
            </div>
          ) : (
            pendingQueue.map((inc) => (
              <AnalysisRow key={inc.id} incident={inc} navigate={navigate} />
            ))
          )}
        </div>
      </div>
