...
export default function DoctorHub() {
  ...
  const getGreeting = () => {
    ...
    if (hrs < 12) return { text: 'Good Morning', sub: 'Clinical Shift Initiation' };
    if (hrs < 17) return { text: 'Good Afternoon', sub: 'Standard Mid-Day Clinical Handover' };
    return { text: 'Good Evening', sub: 'Night Shift Protocols' };
  };
  ...
  const handlePatientSelect = async (patient, existingQueueId = null) => {
    ...
    // For a fresh search-box selection: look up the patient's most recent
  ...
  const handleQuickAction = (submodule) => {
    ...
    } else if (submodule === 'E-Prescription Pad') {
      navigate('/e-prescriptions', { state: { patient: selectedPatient } });
    }
  };
        ...
        <div style={{ position: 'relative', zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1rem' }}>
            ...
            <span style={{ fontSize: '0.75rem', fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#e0f2fe' }}>
              Integrated Doctors Node
            </span>
          </div>
          
          <h1 style={{ margin: 0, fontSize: '2.5rem', fontWeight: 900, tracking: '-0.025em' }}>
            {greeting.text}, Dr. {user?.fullName?.split(' ')[0] || 'Doctor'} ⚕️
          </h1>
          <p style={{ marginTop: '0.5rem', fontSize: '1rem', color: '#bae6fd', fontWeight: 500, maxWidth: '640px' }}>
            Welcome to the Clinical Command Center. {greeting.sub}. Review assessments, issue prescriptions, and manage patient care.
          </p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { 
                  ...
                  desc: 'Review triage, add progress notes, verify diagnoses, and manage patient care in a continuous document.',
                ...
                { 
                  ...
                  desc: 'Issue digital prescriptions securely. Integrated with pharmacy inventory and clinical AI support.',
