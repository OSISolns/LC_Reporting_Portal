...
import { RefreshCw, Plus, LogIn, X, CalendarClock, Sparkles } from 'lucide-react';
...
const ImagingWorklist = () => {
  ...
  useEffect(() => {
    ...
    getProviders().then((r) => setProviders(r.data.data || [])).catch(() => {});
  }, []);

  const handleLuminaIndication = async () => {
    setAiGenerating(true);
    try {
      const res = await generateClinicalIndication({
        modality: form.modality,
        exam_region: form.exam_region,
        exam_type: form.exam_type,
        patient_age: form.patient_age,
        patient_sex: form.patient_sex,
        clinical_indication: form.clinical_indication,
      });
      if (res.data?.data?.indication) {
        setForm((f) => ({ ...f, clinical_indication: res.data.data.indication }));
        toast.success('Clinical indication generated by Lumina AI!');
      }
    } catch (err) {
      toast.error('Failed to generate indication with Lumina AI.');
    } finally {
        ...
        <form onSubmit={submit} className="bg-white rounded-xl border border-slate-200 p-5 mb-5 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <PatientAutocomplete
              ...
              onPatientSelect={(p) => setForm((f) => ({
                ...
                patient_sex: normalizeSex(p.gender || p.sex || f.patient_sex),
                referring_provider: p.referrer_name || f.referring_provider,
              }))}
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
              placeholder="Select or type doctor's name..."
            />
          ...
          <div className="md:col-span-2">
            <div className="flex items-center justify-between mb-1">
              <label className="block text-xs font-semibold text-slate-500">Clinical indication</label>
              <button
                type="button"
                onClick={handleLuminaIndication}
                disabled={aiGenerating}
                className="inline-flex items-center gap-1 text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 px-2 py-0.5 rounded-md transition-all disabled:opacity-50"
              >
                <Sparkles size={13} className={aiGenerating ? 'animate-spin' : 'text-indigo-500'} />
                {aiGenerating ? 'Lumina AI Thinking…' : 'Lumina AI Auto-Fill'}
              </button>
            </div>
            <textarea
              ...
              rows={2}
              placeholder="Type or click 'Lumina AI Auto-Fill' to generate indication..."
