...
const EPrescriptions = () => {
  const { user } = useAuth();
  const [patientInfo, setPatientInfo] = useState({ name: '', dob: '', id: '', date: new Date().toISOString().split('T')[0], diagnosis: '' });
  ...
  const handleSave = () => {
    ...
    toast.success('Prescription saved to patient record successfully!');
    // Reset after save
    setPatientInfo({ name: '', dob: '', id: '', date: new Date().toISOString().split('T')[0], diagnosis: '' });
      ...
      <div className="print:hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 w-full md:w-auto">
            ...
            <button 
              onClick={handleSave}
              className="px-6 py-3.5 rounded-2xl bg-[#1B669E] text-white font-bold text-base flex items-center gap-2 shadow-lg shadow-[#1B669E]/20 hover:bg-[#155482] transition-all active:scale-95 hover:-translate-y-0.5"
            >
              <Save size={20} /> Save & Issue
            </button>
          </div>
        </div>
              ...
              <div key={index} className="p-8 border-b border-slate-100 last:border-0 relative group transition-colors hover:bg-slate-50/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-6 pr-14">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input type="text" name="route" value={med.route} onChange={(e) => handleMedChange(index, e)} placeholder="e.g. PO" className="w-full px-5 py-3.5 rounded-xl border-2 border-slate-200 focus:border-[#1B669E] focus:ring-4 focus:ring-[#1B669E]/10 outline-none text-base transition-all font-medium" />
                  </div>
                  <div className="md:col-span-3">
                    <label className="block text-sm font-bold text-slate-500 uppercase tracking-wider mb-3">Frequency</label>
                    <input type="text" name="frequency" value={med.frequency} onChange={(e) => handleMedChange(index, e)} placeholder="e.g. TID" className="w-full px-5 py-3.5 rounded-xl border-2 border-slate-200 focus:border-[#1B669E] focus:ring-4 focus:ring-[#1B669E]/10 outline-none text-base transition-all font-medium" />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-sm font-bold text-slate-500 uppercase tracking-wider mb-3">Duration</label>
                    <input type="text" name="duration" value={med.duration} onChange={(e) => handleMedChange(index, e)} placeholder="e.g. 7 Days" className="w-full px-5 py-3.5 rounded-xl border-2 border-slate-200 focus:border-[#1B669E] focus:ring-4 focus:ring-[#1B669E]/10 outline-none text-base transition-all font-medium" />
                  </div>
                </div>
                <div className="pr-14">
          ...
          {medications.map((med, index) => (
            <div key={index} className="med-row">
              ...
              <div className="pl-6 text-sm text-gray-700">
                <p className="m-0 mb-1"><span className="font-bold">Sig:</span> {med.route} {med.frequency} {med.duration ? `for ${med.duration}` : ''}</p>
