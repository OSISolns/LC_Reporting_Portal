...
(async () => {
  try {
    ...
    const result = await page.evaluate(() => {
      ...
      if (!grid) return 'No grid after view';
      const headerRows = Array.from(grid.querySelectorAll('tr.Header'));
      const headerRow = headerRows[1];
      if (!headerRow) return 'No header row after view';
      return Array.from(headerRow.querySelectorAll('th, td')).map((cell, idx) => ({
        idx,
    ...
    console.log('Headers:', JSON.stringify(result, null, 2));
    await browser.close();
  } catch (err) {
